...
p <- p + labs(title = 'DYRK3 Enriched top5 GO') +
  theme(
...
p
ggsave("DYRK3_gene_GO.pdf", p, w=11, h=9)
...
p <- p + labs(title = 'DYRK3 Enriched top 5 simplyGO') +
  theme(
    plot.title = element_text(size = 14, face = 'bold'),
    axis.title = element_text(size = 13),
    axis.text = element_text(size = 11),
...
p
ggsave("DYRK3_gene_simplyGO.pdf", p, w=11, h=9)
