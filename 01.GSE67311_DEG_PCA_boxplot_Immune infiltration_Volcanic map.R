...
p <- ggplot(data=dat, aes(x=logFC, y=-log10(P.Value), color=regulated)) + 
  ...
  scale_colour_manual(
    ...
    labels = c(down = "down", stable = "normal", up = "up")
  ) +
  geom_vline(xintercept = 0, linetype = 2, color = "black", size = 0.8) +
  geom_hline(yintercept = -log10(0.05), linetype = 2, color = "black", size = 0.8)
p


##添加基因名圈DYRK3/RGS17/ARHGEF37
which(dat$symbol=="DYRK3")#1054
which(dat$symbol=="RGS17")#20
which(dat$symbol=="ARHGEF37")#723

label <- dat[c(1054,20,723),]
